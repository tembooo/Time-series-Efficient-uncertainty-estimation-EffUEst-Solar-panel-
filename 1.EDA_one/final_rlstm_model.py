...
# Load top trial parameters
print("🔹 Loading top trial parameters...")
with open("top_trials_params.txt", "r") as f:
    top_params = ast.literal_eval(f.read())
...
class NLLCallback(callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i in range(0, len(self.validation_data[0]), batch_size * 2):
            batch_pred = self.model.predict(self.validation_data[0][i:i + batch_size * 2], batch_size=batch_size, verbose=0)
...
for i, hp in enumerate(top_params):
    print(f"\n🔹 Training RLSTM model {i+1} with hyperparameters: {hp}")
    ...
    model.fit(
        ...
        callbacks=[
            ...
            callbacks.ModelCheckpoint(f'final_rlstm_model_{i+1}.h5', monitor='val_loss', save_best_only=True, mode='min')
        ],
        verbose=2
    )
...
for i, model in enumerate(ensemble_models):
    ...
    y_pred = []
    for j in range(0, len(X_test), batch_size * 2):
        batch_pred = model.predict(X_test[j:j + batch_size * 2], batch_size=batch_size, verbose=0)
        y_pred.append(batch_pred)
    y_pred = np.concatenate(y_pred, axis=0)
    mu_pred_scaled = y_pred[:, :, 0]
    log_var_pred = y_pred[:, :, 1]
    var_pred = tf.nn.softplus(log_var_pred).numpy() + 1e-6
    mu_preds.append(mu_pred_scaled)
    var_preds.append(var_pred)
    tf.keras.backend.clear_session()
    gc.collect()

# Combine predictions
mu_pred_scaled = np.mean(mu_preds, axis=0)
var_pred = np.mean(var_preds, axis=0) + np.var(mu_preds, axis=0)  # Total variance includes predictive variance and model variance
...
print(f"\n📊 Ensemble RLSTM Model Evaluation:")
print(f"MAE:  {mae:.2f}")
...
with pd.ExcelWriter("final_rlstm_predictions.xlsx") as writer:
...
print("✅ Saved final_rlstm_predictions.xlsx")
...
tf.keras.backend.clear_session()
gc.collect()
print("✅ Ensemble RLSTM training complete")
